...
export class AuthService {
  ...
  private setUser(user: any) {
    ...
    this.authState.next({isLoggedIn: true, id, username, email, event: 'signedin'});
  }

  signOut() {
    Auth.signOut({global: true}).catch(err => console.log(err));
    this.authState.next({isLoggedIn: false, email: null, username: null, id: null, event: 'signedout'});
  }
}
